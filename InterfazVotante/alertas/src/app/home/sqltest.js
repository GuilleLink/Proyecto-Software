...
function queryDatabase_Test() {
  ...
  var request = new Request("SELECT * FROM mesas", function(
    err,
    rowCount,
    rows
  ) {
...
var DBresults = {
  //para guardar los resultados y mandar el objeto al ts
  columname: [],
  colvalue: []
};
...
function CheckEmpadronamiento(dpi) {
  ...
  // Read all rows from table
  var request = new Request("SELECT * FROM padronE WHERE dpi =" + dpi, function(
    err,
    rowCount,
    rows
  ) {
